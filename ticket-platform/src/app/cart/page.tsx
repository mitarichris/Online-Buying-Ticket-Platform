  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center">
        ...
        <p className="mt-2 text-gray-500">Looks like you haven&apos;t added any tickets yet.</p>
        <Link href="/events">
    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-gray-500">{items.length} item(s) in your cart</p>
        </div>
          ...
          <Card key={`${item.eventId}-${item.ticketTypeId}`}>
            <CardContent className="flex items-center gap-4 p-4">
              ...
              <div className="flex-1 min-w-0">
                <Link href={`/events/${item.eventId}`} className="font-semibold text-gray-900 hover:text-indigo-600">
                  {item.eventTitle}
                </Link>
                <p className="text-sm text-gray-500">{formatDate(item.eventDate)}</p>
                <p className="text-sm text-gray-500">{item.ticketTypeName}</p>
                <p className="mt-1 font-medium text-indigo-600">{formatCurrency(item.price)} each</p>
              </div>
